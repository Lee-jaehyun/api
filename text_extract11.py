...
def ocr_prsc( ): #base64_data
    start = time.time()

  #  data = BytesIO(base64.b64decode(base64_data))

   # img = Image.open(data)
    img = cv2.imread("../../Desktop/image1.jpeg")
...
#angle = determine_skew(grayscale)
    angle = determine_skew(grayscale[int(h/4):int(h*3/4), int(w/4):int(w*2/3)])   #[int(h/3):, int(w/4):int(w*2/3)]
...
#    mask = np.zeros(src_bin.shape, dtype=np.uint8)
    ...
    grayscale1 = cv2.resize(grayscale, None, fx=2.5, fy=2.5, interpolation=cv2.INTER_CUBIC)
    #thr = cv2.threshold(grayscale1, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)[1]
    start_ocr = time.time()
